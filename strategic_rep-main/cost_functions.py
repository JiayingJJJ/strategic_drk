...
class MixWeightedLinearSumSquareCostFunction(CostFunction):
    ...
    def solve_optimization_min_cost(self, model, x, tol=0.00001):
        """
        Solves for x_t that minimizes the cost function under the defined constraints.

        :param x: Current feature vector (numpy array).
        :param a: Weight matrix or vector used in cost calculation.
        :param epsilon: Smoothing factor in cost calculation.
        :param cost_factor: Cost scaling factor.
        :param tol: Small tolerance for optimization constraints.
        :return: Optimal x_t and computed cost.
        """
        ...
        # print((1 - self.epsilon) * self.a.T @ (x_t - x))
        # constrains = [x_t @ model.coef_[0] >= x @ model.coef_[0] + tol]
        prob = cp.Problem(func_to_solve, constrains)
        try:
            prob.solve()
            if x_t is None:
    ...
    def check_change_condition_2(self, trained_model, x_tag, cost_result):
        # print(cost_result.value)
        return cost_result.value < 2
    
    def maximize_features_against_binary_model_origin(self, x: np.array, trained_model, tolerance=0.00001,
                                               use_spare_cost=False):
        # x_tag, cost_result = self.solve_problem_min_cost_s_t_model(trained_model, x, tolerance)
        x_tag, cost_result = self.solve_optimization_min_cost(trained_model, x, tolerance)
        ...
        if self.check_change_condition(trained_model, x_tag, cost_result):
            print(x_tag.value)
            print(x)
            return x_tag.value, cost_result.value
        else:
            print("___________________")
            return x, cost_result.value
    ...
    def maximize_features_against_binary_model(self, x: np.array, trained_model, tolerance=0.00001,
        ...
        # if self.check_change_condition(trained_model, x_tag, cost_result):
        #     return x_tag.value
        if self.check_change_condition_2(trained_model, x_tag, cost_result):
            print(x_tag.value)
            print(x)
            return x_tag.value, cost_result.value
        else:
