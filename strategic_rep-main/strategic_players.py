...
def get_player_movements_and_update_modify_data_df(cost_func: MixWeightedLinearSumSquareCostFunction,
    ...
    '''
    z, cost = cost_func.maximize_features_against_binary_model(x, f_hat, use_spare_cost=True) # nearest point
    z_2, cost_2 = cost_func.maximize_features_against_binary_model_origin(x, f_hat, use_spare_cost=True) #this is the max outcome
    # modify_data.loc[index] = z
    c_2 = 5 * (1 - 0.8) - (-10) * 0.8 + cost_2
    c = 5 * (1 - 0.8) - (-10) * 0.8 + cost
    if c > c_2:
        z = z_2
    did_move = True
    modify_data.loc[index] = z if len(z) > 1 else z[0]
